...
export default async function decayToadStats( req:NextApiRequest, res:NextApiResponse) {
    ...
    if (method === 'PATCH') {
        ...
        if (password === process.env.DECAY_API_TOKEN) {
            res.status(200).json({message: `Authorized request ${passwordJSON}....${passwordKey}.... ${password}`})
            ...
            res.status(500).json({message: `Unauthorized request ${passwordJSON}....${passwordKey}.... ${password}`})
        }       
    } else {
